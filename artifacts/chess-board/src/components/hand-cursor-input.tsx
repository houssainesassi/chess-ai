...
export function HandCursorInput({
  ...
  useEffect(() => {
    if (enabled) { startHandTracking(); } else { stopAll(); setLoadingState("idle"); }
    return () => stopAll();
  }, [enabled]);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {loadingState === "error" && (
        <button onClick={startHandTracking} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
          <RefreshCw size={11} />
          Retry
        </button>
      )}
