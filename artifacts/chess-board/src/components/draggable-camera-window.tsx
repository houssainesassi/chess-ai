...
export function DraggableCameraWindow({ title, color = "text-muted-foreground", onClose, children, defaultX, defaultY }: Props) {
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (!touch) return;
      setPos({
        x: Math.max(0, Math.min(window.innerWidth - 300, dragRef.current.origX + touch.clientX - dragRef.current.startX)),
  ...
  return (
    <div className="fixed z-50 w-80 bg-card border border-border/60 rounded-xl shadow-2xl overflow-hidden select-none" style={{ left: pos.x, top: pos.y }}>
      <div
