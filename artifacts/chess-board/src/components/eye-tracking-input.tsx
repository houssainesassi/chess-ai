...
interface CalibrationData { minX: number; maxX: number; minY: number; maxY: number; }
...
function gazeToSquare(gaze: { x: number; y: number }, calib: CalibrationData): string {
...
export function EyeTrackingInput({
  ...
  const stopAll = useCallback(() => {
    ...
    setGazeSquare(null); setFromSquare(null); setDwellProgress(0);
    onSquareHover(null); onSquareSelect(null);
  ...
  const captureCalibPoint = useCallback(() => {
    ...
    if (calibStep === "a8") {
      ...
      const newCalib: CalibrationData = { minX: pending.minX ?? DEFAULT_CALIB.minX, minY: pending.minY ?? DEFAULT_CALIB.minY, maxX: g.x, maxY: g.y };
  ...
  const resetCalib = useCallback(() => { calibRef.current = { ...DEFAULT_CALIB }; setCalib({ ...DEFAULT_CALIB }); setCalibStep(null); }, []);
  ...
  const runDetectionLoop = useCallback(() => {
    ...
    if (video.readyState < 2) { rafRef.current = requestAnimationFrame(runDetectionLoop); return; }
    ...
    let results: any;
    try { results = landmarker.detectForVideo(video, performance.now()); } catch { rafRef.current = requestAnimationFrame(runDetectionLoop); return; }
    ...
    if (faceLandmarks.length > 0) {
      ...
      if (rawGaze) {
        ...
        smoothedGazeRef.current = prev
          ? { x: SMOOTH_ALPHA * rawGaze.x + (1 - SMOOTH_ALPHA) * prev.x, y: SMOOTH_ALPHA * rawGaze.y + (1 - SMOOTH_ALPHA) * prev.y }
          : rawGaze;
        ...
        if (square !== dwellSquareRef.current) {
          ...
          if (progress >= 1 && !isMoveLocked()) {
            dwellStartRef.current = now + 99999;
            if (!fromSquareRef.current) {
              const chess = new Chess(currentFenRef.current);
              const piece = chess.get(square as any);
              if (piece) { fromSquareRef.current = square; setFromSquare(square); onSquareSelect(square); }
            } else if (square === fromSquareRef.current) {
              fromSquareRef.current = null; setFromSquare(null); onSquareSelect(null);
            } else {
              const chess = new Chess(currentFenRef.current);
              try {
                const move = chess.move({ from: fromSquareRef.current as any, to: square as any, promotion: "q" });
                if (move) { const uci = move.from + move.to + (move.promotion ?? ""); setLastMove(move.from + move.to); onMove(uci, "eye"); }
              } catch {}
              fromSquareRef.current = null; setFromSquare(null); onSquareSelect(null);
            }
          }
        ...
        ctx.save();
        ctx.beginPath(); ctx.arc(dispX, dispY, 12, 0, Math.PI * 2); ctx.strokeStyle = "rgba(34,211,238,0.6)"; ctx.lineWidth = 2; ctx.stroke();
        if (progress > 0 && progress < 1) {
          ctx.beginPath(); ctx.arc(dispX, dispY, 12, -Math.PI / 2, -Math.PI / 2 + progress * Math.PI * 2); ctx.strokeStyle = "#22d3ee"; ctx.lineWidth = 3; ctx.stroke();
        }
        ctx.beginPath(); ctx.arc(dispX, dispY, 4, 0, Math.PI * 2); ctx.fillStyle = "#22d3ee"; ctx.fill();
        ctx.fillStyle = "rgba(0,0,0,0.6)"; ctx.fillRect(dispX - 18, dispY + 16, 36, 16);
        ctx.fillStyle = "#22d3ee"; ctx.font = "bold 11px monospace"; ctx.textAlign = "center"; ctx.fillText(square, dispX, dispY + 27);
        ctx.restore();

        for (const irisIdx of [LEFT_IRIS, RIGHT_IRIS]) {
          const il = lm[irisIdx];
          const ix = (1 - il.x) * W; const iy = il.y * H;
          ctx.beginPath(); ctx.arc(ix, iy, 5, 0, Math.PI * 2); ctx.strokeStyle = "rgba(34,211,238,0.9)"; ctx.lineWidth = 2; ctx.stroke();
        }
        ...
        for (let i = 0; i < FACE_OUTLINE.length; i++) {
          const p = lm[FACE_OUTLINE[i]]; const px = (1 - p.x) * W; const py = p.y * H;
          if (i === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py);
        }
        ctx.closePath(); ctx.strokeStyle = "rgba(99,202,183,0.25)"; ctx.lineWidth = 1; ctx.stroke();
      }
    } else {
      if (smoothedGazeRef.current) {
        smoothedGazeRef.current = null;
        setGazeSquare(null); setDwellProgress(0); onSquareHover(null);
      }
    }

    const cw = W / 8; const ch = H / 8;
  ...
  const startEyeTracking = useCallback(async () => {
    ...
    try {
      ...
      const msg = err?.name === "NotAllowedError" ? "Camera permission denied." : err?.name === "NotFoundError" ? "No camera found." : `Failed to initialize: ${err?.message ?? "unknown error"}`;
      setErrorMsg(msg); setLoadingState("error");
    }
  }, [runDetectionLoop]);

  useEffect(() => {
    if (enabled) { startEyeTracking(); } else { stopAll(); setLoadingState("idle"); }
    return () => stopAll();
  }, [enabled]);

  useEffect(() => {
    if (enabled && loadingState === "ready") { cancelAnimationFrame(rafRef.current); rafRef.current = requestAnimationFrame(runDetectionLoop); }
  ...
  return (
    <div className="flex flex-col gap-2">
        ...
        {calibStep && (
          <div className="absolute inset-0 bg-black/75 flex flex-col items-center justify-center gap-3 px-3">
            ...
            <p className="text-xs text-cyan-300 text-center leading-snug">
              {calibStep === "a8" ? "Look at the top-left corner of the board (a8), then click Capture." : "Now look at the bottom-right corner (h1), then click Capture."}
            </p>
            <button onClick={captureCalibPoint} className="px-3 py-1.5 bg-cyan-500 hover:bg-cyan-400 text-black text-xs font-bold rounded-full transition-colors">Capture</button>
            <button onClick={() => setCalibStep(null)} className="text-[10px] text-muted-foreground underline">Cancel</button>
          </div>
        )}
      </div>
      ...
      {loadingState === "ready" && (
        <div className="flex flex-col gap-1.5 px-1">
          <div className="flex items-center gap-2 text-[10px]">
            ...
            {dwellProgress > 0 && (
              <div className="ml-auto h-1.5 w-16 bg-border/40 rounded-full overflow-hidden">
                <div className="h-full bg-cyan-400 rounded-full transition-all duration-100" style={{ width: `${dwellProgress * 100}%` }} />
              </div>
            )}
          </div>
          {lastMove && (
          ...
          <div className="flex items-center gap-2 mt-0.5">
            {!calibStep && (
              <button onClick={() => { pendingCalibRef.current = {}; setCalibStep("a8"); }} className="flex items-center gap-1 text-[10px] text-cyan-400/70 hover:text-cyan-400 transition-colors">
                <Crosshair size={10} />
                Calibrate gaze
              </button>
            )}
            {(calib.minX !== DEFAULT_CALIB.minX || calib.maxX !== DEFAULT_CALIB.maxX) && (
              <button onClick={resetCalib} className="text-[10px] text-muted-foreground/50 hover:text-muted-foreground transition-colors ml-auto">Reset calib</button>
            )}
          </div>
      ...
      {loadingState === "error" && (
        <button onClick={startEyeTracking} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
          <RefreshCw size={11} />
          Retry
        </button>
      )}
